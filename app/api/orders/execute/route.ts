...
export async function POST(req: NextRequest) {
  try {
    ...
    // Registrar recibo on-chain (no bloquea si falla)
    let receiptTx = `mock-receipt-${Date.now()}`;
    try {
      const { recordReceiptOnChain } = await import('@backend/services/anchorReceipt');
      receiptTx = await recordReceiptOnChain({
        simulationId,
        amountLamports: Math.round((cached.intent.amount ?? 0) * 1e9),
        tokenFrom: cached.intent.tokenFrom ?? 'USDC',
        tokenTo: cached.intent.tokenTo ?? 'SOL',
        confirmationType: confirmation.type,
      });
    } catch (anchorErr) {
      console.error('[execute] Anchor receipt failed (non-blocking):', anchorErr);
    }
    ...
    query(
      ...
      [userId, `${userId}@vibebroker.dev`, simulationId, cached.intent.amount??0,
       cached.intent.tokenFrom??'USDC', cached.intent.tokenTo??'SOL',
       confirmation.type, confirmation.type==='voice'?'voice':'passkey', txHash, receiptTx],
    ...
    if (confirmation.type === 'voice')
      query(`UPDATE users SET voice_daily_used=voice_daily_used+$1 WHERE id=$2 OR email=$3`,
        [cached.intent.amount??0, userId, `${userId}@vibebroker.dev`]).catch(()=>null);

    deleteSimulation(simulationId);
    return NextResponse.json({ txHash, status: 'submitted', receiptId: receiptTx });
  } catch (e) {
