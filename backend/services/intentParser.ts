...
const TOKENS: Record<string, string> = {
  ...
  matic:'MATIC', polygon:'MATIC', avax:'AVAX', avalanche:'AVAX',
  ftm:'FTM', fantom:'FTM', arb:'ARB', arbitrum:'ARB',
  op:'OP', optimism:'OP', base:'BASE',
};

const CHAINS: Record<string, string> = {
  solana:'Solana', sol:'Solana', ethereum:'Ethereum', eth:'Ethereum',
  binance:'BNBChain', bnb:'BNBChain', polygon:'Polygon', matic:'Polygon',
  avalanche:'Avalanche', avax:'Avalanche', fantom:'Fantom', ftm:'Fantom',
...
function detectChain(tokenSymbol: string): string | undefined {
  return CHAINS[tokenSymbol.toLowerCase()];
}
...
export function parseIntent(text: string, asrConfidence = 1.0): ParsedIntent {
  ...
  for (const p of BRIDGE) { const m = t.match(p); if (m) { const tf = norm(m[2]); const tt = norm(m[3]); return { ...b, action:'bridge', amount:parseAmt(m[1]), tokenFrom:tf, tokenTo:tt, chain:detectChain(m[3]) }; } }
  for (const p of SWAP)   { const m = t.match(p); if (m) { const tf = norm(m[2]); const tt = norm(m[3]); return { ...b, action:'swap',   amount:parseAmt(m[1]), tokenFrom:tf, tokenTo:tt, chain:detectChain(m[2]) }; } }
  for (const p of BUY)    { const m = t.match(p); if (m) { const tt = norm(m[2]); return { ...b, action:'buy',    amount:parseAmt(m[1]), tokenFrom:'USDC',     tokenTo:tt, chain:detectChain(m[2]) }; } }
  for (const p of SELL)   { const m = t.match(p); if (m) { const tf = norm(m[2]); return { ...b, action:'sell',   amount:parseAmt(m[1]), tokenFrom:tf, tokenTo:'USDC', chain:detectChain(m[2]) }; } }
